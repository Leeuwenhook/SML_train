...
cp_grid <- expand.grid(cp = seq(0.001, 0.013, by = 0.0002))

# Train the model with complexity parameter tuning
model <- train(formula, data = trainingData, method = "rpart",
               trControl = train_control, tuneGrid = cp_grid,metric='RMSE')
...
#' 
## ----------------------------------------------
write.csv(adjusted_predictions, file = "film_rating_predictions_group_F_week_1.csv", row.names=FALSE)
